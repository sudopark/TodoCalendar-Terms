...
日历小组件存在的意义，就是让你不必打开日历。To-do Calendar 提供了一整套小组件，你可以挑一个符合自己查看习惯的 —— 只看下一件事、看一整个月，或者看带勾选框、能直接点的待办列表。
...
| 小组件 | 显示内容 | 尺寸 |
|---|---|---|
| 今天 + 日历 | 今天的摘要挨着月视图网格 | 中 |
| 事件 + 日历 | 接下来的事件挨着月视图网格 | 中 |
| 事件 + 最重要 | 接下来的事件挨着你钉住的事件 | 中 |
| 日历 + 日历 | 两个月并排显示 | 中 |
...
把一个事件放到锁定屏幕上，看着距离它的时间一点点减少，灵动岛里也是同一个视图。在事件的更多菜单里选 **在锁定屏幕上显示** 就可以了。

- 只有 8 小时内开始的事件可以显示。
- 一次只能有一个 —— 再选一个新的时，会问你要不要替换当前那个。
- 待办可以直接在实时活动上完成。
