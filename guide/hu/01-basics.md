...
A havi naptár a kezdőképernyő. Húzza balra vagy jobbra az ujját a hónapok közti váltáshoz, és koppintson egy napra, hogy alatta megnyíljon az aznapi eseménylista.

- Minden nap eseményenként egy színes sávot mutat, és **+N** jelzést, ha a napon több esemény van, mint amennyi a sorban elfér.
...
Az, hogy mennyit mutasson egy nap, Önön múlik: az eseményenkénti részletesség, a szövegméret, a színek, az ünnepnevek és a holdnaptár. A [Személyre szabás](./05-personalization.md) minden beállítást néven nevez.

---

## Feladatok és programok

Az alkalmazásban kétféle esemény van, és a különbség az, hogy kipipálja-e az adott dolgot.
...
Minden eseményhez tartozhat **Helyszín**, **Hivatkozás** és **Jegyzet**. A helyszínhez térkép-előnézet jár, és egyetlen koppintással megnyílik a kedvenc térképalkalmazásában; a hivatkozásnak saját előnézete van.

---

## Események hozzáadása

Három módja van egy esemény hozzáadásának, attól függően, mennyit szeretne gépelni:

- **Gyors hozzáadás** — a beviteli mező a napi lista alján. Írjon be egy nevet, nyomjon entert, és létrejön a feladat.
...
Ahelyett, hogy legördülő menükből kellene összeraknia egy ismétlési szabályt, az alkalmazás elolvassa a kiválasztott dátumot, és kész lehetőségeket kínál hozzá. Válasszon egy csütörtököt, és a lista a **Minden csütörtökön** és a **Minden hónap harmadik csütörtökén** lehetőséget kínálja.

**Gyakori időközök**

- Minden nap
- Minden héten · Minden 2. héten · Minden 3. héten · Minden 4. héten — a hét ugyanazon napján, mint az esemény
...
- Minden hónap első / második / harmadik / negyedik / utolsó **csütörtökén** — a hét napja a kiválasztott dátumból töltődik ki, így egy pénteki eseménynél a **Minden hónap utolsó péntekén** jelenik meg

**Ismétlés vége**

Ha kiválasztott egy ismétlést, adja meg, mikor álljon le: a **Soha** végtelenül futtatja, az **Ekkor** rögzített befejező dátumot ad, az **Ezután** pedig az alkalmak számát korlátozza.

Az ismétlődő feladatok másképp viselkednek, mint az ismétlődő programok:

- Egy be nem fejezett ismétlődés a mai naptárban akkor is látható marad, ha az időpontja már elmúlt — magától nem lép tovább a következő alkalomra.
...
Csatlakoztatott külső naptáron lévő eseményeknél a holdnaptár szerinti lehetőség nem jelenik meg — a külső naptáraknak nincs hova elmenteniük a holdnaptár szerinti ismétlési szabályt.
...
- Az **Alapértelmezett eseménytípus** beállításával az új események a megfelelő helyre kerülnek anélkül, hogy minden alkalommal választania kellene.
...
Az időponthoz kötött és az egész napos események alapértelmezései külön állíthatók a Beállításokban, így az új eseményeken már eleve be vannak állítva az értesítések. Az értesítésekhez értesítési engedély kell; ha ki van kapcsolva, az alkalmazás elirányítja az iOS Beállításokhoz.
...
Azok a feladatok, amelyeknek az időpontja már elmúlt, de nem lettek befejezve, a naptár tetején a **Befejezetlen feladatok** szakaszban gyűlnek össze, így egy elmulasztott feladat nem marad elveszve egy elmúlt dátumon.
